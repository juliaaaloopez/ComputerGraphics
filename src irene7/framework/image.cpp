        ...
        if (table[y].minX < table[y].maxX) {
            for (int x = table[y].minX; x < table[y].maxX; ++x) {
                ...
                if (pixelBarycentricCoords.x >= 0 && pixelBarycentricCoords.y >= 0 && pixelBarycentricCoords.z >= 0) {
                    ...
                    Color finalColor = Color::RED; // Initialize final color as black
                    ...
                    if (isTextured) {
                        if (texture != nullptr) {
                            ...
                            // Map u, v to texture space
                            u = u * (texture->width - 1);
                            v = v * (texture->height - 1);

                            // Get texture color
                            Color textureColor = texture->GetPixelSafe(u, v);

                            // Blend texture color with final color
                            finalColor = textureColor;
                        }
                    }

                    if (!isOcluded) {
                        // Perform depth testing and update z-buffer
