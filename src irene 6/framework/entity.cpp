...
void Entity::Render(Image* framebuffer, Camera* camera, FloatImage* zBuffer, bool isInterpolated, bool isOcluded, bool isTextured) {
    ...
    for (int i = 0; i < vertices.size(); i += 3) {
        ...
        if (!negZ0 && !negZ1 && !negZ2) { // if the triangle is inside the frame
            ...
            if (isInterpolated == false) {
                Vector2 v0 = { p0.x, p0.y };
                Vector2 v1 = { p1.x, p1.y };
                Vector2 v2 = { p2.x, p2.y };
                framebuffer->DrawTriangle(v0, v1, v2, color0, 1, color0); //triangle in plain color
            }
            ...
            if (isInterpolated==true){
                ...
                framebuffer->DrawTriangleInterpolated(p0, p1, p2, color0, color1, color2, zBuffer, texture, uv0, uv1, uv2, 1, 0, 0); //the boolean isInterpolated is set to true
            }
            ...
            if (isOcluded) {
                ...
                framebuffer->DrawTriangleInterpolated(p0, p1, p2, color0, color1, color2, zBuffer, texture, uv0, uv1, uv2, 0, 1, 0); //the boolean isOcluded is set to true
            ...
            if (!isOcluded) {
                ...
                framebuffer->DrawTriangleInterpolated(p0, p1, p2, color0, color1, color2, zBuffer, texture, uv0, uv1, uv2, 0, 0, 0); //the boolean isOcluded is set to false
            ...
            if (isTextured) {
                ...
                framebuffer->DrawTriangleInterpolated(p0, p1, p2, color0, color1, color2, zBuffer, texture, uv0, uv1, uv2, 0, 0, 1); //the boolean isTextured is set to true
            }
            if (!isTextured) {
                Vector2 uv0 = mesh->GetUVs()[i];
                Vector2 uv1 = mesh->GetUVs()[i + 1];
                ...
                framebuffer->DrawTriangleInterpolated(p0, p1, p2, color0, color1, color2, zBuffer, texture, uv0, uv1, uv2, 0, 0, 0); //the boolean isTextured is set to false
