...
void Camera::Orbit(Vector3 target, float angle) {

	Vector3 to_target = target - eye; //vector from target to eye

	float distance = to_target.Length(); //distance from the eye to the target

	//update the eye position to orbit
	eye.x = target.x + distance * cos(angle);
	eye.z = target.z + distance * sin(angle);
	//eye.y is the height that doesn't change
